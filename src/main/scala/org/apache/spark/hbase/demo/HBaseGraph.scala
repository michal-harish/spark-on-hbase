...
trait HBaseGraph extends AGraph[HE] with Utils {
  ...
  /**
   * STATS = spark accumulator map for collecting stats asynchronously from map tasks
   */
   ...
   */
  final def fromTextList(he: HE, textFile: RDD[String], keySpace: String): NETWORK = {
   ...
   */
  final def fromList(he: HE, in: RDD[Seq[HKey]]): NETWORK = {
    deduplicate(
      in.flatMap(a => {
  ...
  class HGraph(tableName: String, numberOfRegions: Int) extends HBaseTable(hbaseConfig, tableName: String, numberOfRegions
    ...
    def incrementalNetBSP(adj: NETWORK, lookup: HBaseLookup[EDGES, (EDGES, EDGES)], numSteps: Int): BSP_RESULT = {
